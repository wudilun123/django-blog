...
document.addEventListener('click', function (event) {
    ...
    //提交新建分类
    if (elem == makeCategory) createCategory();
    //提交文章
    if (elem == makeTopic) createTopic();
})
    ...
    (async () => {
        ...
        switch (jsonResponse.code) {
            case 200:
                ...
                break;
            case 403:
                myAlert.showAlert(jsonResponse.error, () => window.location.href = `/login-reg/?next=${window.location.href}`);
                break;
            default:
...
function createTopic() {
    (async () => {
        ...
        data.introduce = editor.getText().slice(0, 30);
        ...
        switch (jsonResponse.code) {
            case 200:
                myAlert.showAlert('发表成功！')//待添加一个跳转到文章详情页
